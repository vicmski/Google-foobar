...
she captured six key members of the Bunny Rebellion, and she beat her personal high score in Tetris. To celebrate,
she's ordered cake for everyone - even the lowliest of minions! But competition among minions is fierce, and if you
don't cut exactly equal slices of cake for everyone, you'll get in big trouble.
...
def solution(s):
    ...
    for (index, letter) in enumerate(s):
        if letter == s[index % match_length]:
            if not (index + 1) % match_length:
                result = result + 1
        else:
            match_length = index + (1 if letter != s[0] else 0)
            result = 1
